...
class Dataloader():
    ...
    def yield_tokens(self):
        act_seq = self.df.groupby("case_id").apply(lambda x: self.process_seq(x.activities.to_list()))
        for act in act_seq:
            yield act
    ...
    def get_dataset(self, num_test_ex=1000, vocab=None, debugging=False):
        ...
        train_data = self.batchify(self.data_process(train_raw_data, vocab),
                                   bsz=len(train_act_seq) // self.opt["batch_size"])
        valid_data = self.batchify(self.data_process(valid_raw_data, vocab),
                                   bsz=len(valid_act_seq) // self.opt["batch_size"])
        test_data = self.batchify(self.data_process(test_raw_data, vocab),
                                  bsz=len(test_act_seq) // self.opt["batch_size"])
